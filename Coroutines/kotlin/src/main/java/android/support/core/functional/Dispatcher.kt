...
fun Dispatcher.openClearTop(clazz: KClass<out AppCompatActivity>, vararg args: Pair<String, Any>): Dispatcher {
    when (this) {
        is Fragment -> startActivity(Intent(activity, clazz.java)
            .put(args))
        is Activity -> startActivity(Intent(this, clazz.java)
            .put(args))
        else -> throw IllegalArgumentException("This is not instance of activity or fragment")
    }
    return this
...
fun Dispatcher.startForResult(requestId: Int, clazz: KClass<out Activity>, function: Intent.() -> Unit) {
    when (this) {
        is Fragment -> startActivityForResult(Intent(activity!!, clazz.java).apply(function), requestId)
